    ...
    with open('Discover Weekly 20200420', 'r') as lol:
        ...
        try:
            with open('NAME', 'r') as garbled_name:
                okay_ill_try = secret_name = ''.join(('abcdefghijklmnopqrstuvwxyz '[('abcdefghijklmnopqrstuvwxyz '.index(garbled_letter) - beat_this_one[index % len(beat_this_one)]) % 27] for index, garbled_letter in enumerate(garbled_name.read())))
                print('My secret name is', secret_name, 'or maybe it\'s still secret')
        except Exception:
            secret_name = input('What is your secret name? ').lower()
            garbledeegook = ''.join(('abcdefghijklmnopqrstuvwxyz '[('abcdefghijklmnopqrstuvwxyz '.index(secret_letter) + beat_this_one[index % len(beat_this_one)]) % 27] for index, secret_letter in enumerate(secret_name)))
